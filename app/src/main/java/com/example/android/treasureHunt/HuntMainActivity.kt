 ...
 */
        ...
        val foregroundLocationApproved = (
                PackageManager.PERMISSION_GRANTED ==
                        ActivityCompat.checkSelfPermission(this,
                            Manifest.permission.ACCESS_FINE_LOCATION)) // a check we are used to
        val backgroundPermissionApproved =
            if (runningQOrLater) {
                PackageManager.PERMISSION_GRANTED ==
                        ActivityCompat.checkSelfPermission(
                            this, Manifest.permission.ACCESS_BACKGROUND_LOCATION
                        ) // true if granted on q
            } else {
                true // so, true if not on Q
            }
    ...
    private fun requestForegroundAndBackgroundLocationPermissions() {
        if (foregroundAndBackgroundLocationPermissionApproved())
            return // fun to see when code just goes 'nothing to be done, lets bail'
        ...
        } // so resultCode is made by summing
    ...
    private fun addGeofenceForClue() {
        if (viewModel.geofenceIsActive()) return // another bail out
